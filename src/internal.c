                                                                ...
                                                                word32 size)
{
    ...
    *inOutIdx += OPAQUE24_LEN;
    ...
    ret = ProcessPeerCerts(ssl, certs, NULL, totalCerts);
    if (ret != 0)
        return ret;
            ...
            if (IsEncryptionOn(ssl, 0) && ssl->keys.decryptedCur == 0)
            {
                ...
                if (atomicUser) {
                    ...
                    if (!ssl->options.tls1_3) {
                        ...
                        ret = DecryptTls13(ssl, in->buffer + in->idx,
...
int DecodePrivateKey(WOLFSSL *ssl, word16* length)
{
    ...
    if (ssl->buffers.key == NULL) {
        ...
        ERROR_OUT(NO_PRIVATE_KEY, end);
    }

#ifndef NO_RSA
    /* Allocate a new RSA key. */
    ssl->sigKey = (RsaKey*)XMALLOC(sizeof(RsaKey), ssl->heap, DYNAMIC_TYPE_RSA);
    if (ssl->sigKey == NULL)
        ERROR_OUT(MEMORY_E, end);

    /* Assume RSA. */
    ssl->sigType = DYNAMIC_TYPE_RSA;

    /* Initialize the RSA key ready to take private key. */
...
#ifdef HAVE_ECC
    /* Free any previously allocated keys. */
    if (ssl->sigKey != NULL)
        XFREE(ssl->sigKey, ssl->heap, DYNAMIC_TYPE_RSA);

    /* Allocate a new ECC key. */
    ...
    eccSigKey = (ecc_key*)ssl->sigKey;
    /* Assume ECC. */
    ssl->sigType = DYNAMIC_TYPE_ECC;

    /* Initialize the ECC key ready to take private key. */
    ret = wc_ecc_init_ex(eccSigKey, ssl->heap, ssl->devId);
    if (ret != 0) {
        goto end;
    }
    ...
    ret = wc_EccPrivateKeyDecode(ssl->buffers.key->buffer, &idx, eccSigKey,
    ...
    keySz = wc_ecc_size(eccSigKey);
    if (keySz < ssl->options.minEccKeySz) {
        WOLFSSL_MSG("ECC key size too small");
        ERROR_OUT(ECC_KEY_SIZE_E, end);
    }

    /* Return the maximum signature length. */
    *length = wc_ecc_sig_size(eccSigKey);
#endif
    ...
    switch(ssl->options.keyShareState)
    {
        ...
        case KEYSHARE_BUILD:
        {
            ...
            if (ssl->sigType == DYNAMIC_TYPE_RSA) {
                ...
                c16toa(length, verify + extraSz); /* prepend hdr */
            }
        #endif /* !NO_RSA */
