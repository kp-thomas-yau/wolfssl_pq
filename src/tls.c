...
static int TLSX_KeyShare_GenDhKey(WOLFSSL *ssl, KeyShareEntry* kse)
{
    ...
    ret = DhGenKeyPair(ssl, (byte*)params->p, params->p_len,
                       (byte*)params->g, params->g_len,
                       key, &keySz, keyData, &dataSz);
    if (ret != 0)
        goto end;
...
static int TLSX_KeyShare_ProcessDh(WOLFSSL* ssl, KeyShareEntry* keyShareEntry)
{
    ...
    return DhAgree(ssl,
        (byte*)params->p, params->p_len, (byte*)params->g, params->g_len,
        keyShareEntry->key, &keyShareEntry->keyLen, NULL, 0,
