 ...
 */
static INLINE void EncodeSigAlg(byte hashAlgo, byte sigType, byte* output)
{
    switch (sigType) {
#ifdef HAVE_ECC
 ...
 * sigType   The signature type.
 */
static INLINE void DecodeSigAlg(byte* input, byte* hashAlgo, byte* sigType)
{
    switch (input[0]) {
        /* PSS signatures: 0x080[4-6] */
        /* ED25519: 0x0807 */
...
int SendTls13CertificateVerify(WOLFSSL* ssl)
{
    ...
    switch(ssl->options.keyShareState)
    {
        ...
        case KEYSHARE_BUILD:
        {
            ...
            ret = DecodePrivateKey(ssl, &length);
            if (ret != 0)
                goto exit_scv;

            /* Add signature algorithm. */
            EncodeSigAlg(ssl->suites->hashAlgo, ssl->sigType, verify);

            /* Create the data to be signed. */
            CreateSigData(ssl, sigData, &sigDataSz, 0);
            ...
            if (ssl->sigType == DYNAMIC_TYPE_RSA) {
                ...
                sig->length = CreateRSAEncodedSig(sig->buffer, sigData,
                        sigDataSz, ssl->suites->hashAlgo);
                if (ret != 0)
                    goto exit_scv;
            ...
            if (ssl->sigType == DYNAMIC_TYPE_ECC)
                sig->length = sendSz - idx - HASH_SIG_SIZE - VERIFY_HEADER;
        #endif /* HAVE_ECC */
        ...
        #ifdef HAVE_ECC
           if (ssl->sigType == DYNAMIC_TYPE_ECC) {
                ret = EccSign(ssl, sigData, sigDataSz,
                    verify + HASH_SIG_SIZE + VERIFY_HEADER, &sig->length,
                    (ecc_key*)ssl->sigKey,
            ...
            if (ssl->sigType == DYNAMIC_TYPE_RSA) {
                ...
                ret = RsaSign(ssl, sig->buffer, sig->length,
                    verify + HASH_SIG_SIZE + VERIFY_HEADER, &ssl->sigLen,
                    (RsaKey*)ssl->sigKey,
                    ssl->buffers.key->buffer, ssl->buffers.key->length,
            ...
            c16toa((word16)length, verify + HASH_SIG_SIZE);
        ...
        case KEYSHARE_VERIFY:
        {
            ...
            if (ssl->sigType == DYNAMIC_TYPE_RSA) {
                if (verifySig == NULL) {
                    ...
                    XMEMCPY(verifySig, verify + HASH_SIG_SIZE + VERIFY_HEADER,
                                                                ssl->sigLen);
                }

                /* check for signature faults */
                ret = VerifyRsaSign(ssl, verifySig, ssl->sigLen,
                    sig->buffer, sig->length, (RsaKey*)ssl->sigKey);
            }
        #endif /* !NO_RSA */
        ...
        {
            /* Put the record and handshake headers on. */
            AddTls13Headers(output, length + HASH_SIG_SIZE + VERIFY_HEADER,
            ...
            sendSz = BuildTls13Message(ssl, output,
                                       ...
                                       output + RECORD_HEADER_SZ,
                                       sendSz - RECORD_HEADER_SZ, handshake,
                                       1, 0);
            if (sendSz < 0) {
                ret = sendSz;
                goto exit_scv;
